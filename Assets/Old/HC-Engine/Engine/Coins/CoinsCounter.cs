...
namespace Engine.Coin
{
    public class CoinsCounter
    {
        ...
        public CoinsCounter(TMPro.TextMeshProUGUI text, int totalCoins, float timeFinished = 3, float deltaTimeUpdate = 0.1f)
        {
            _text = text;
        ...
        public virtual bool UpdateCount(int totalCoins, int amount)
        {
            if (amount < 0)
            {
                ...
                _totalCoins = totalCoins;
                _text.text = Normal.Normalization.NormalizeScore(_totalCoins);
                isCounting = false;
                return true;
            }
